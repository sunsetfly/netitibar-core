...
Kural: İlk local baseline Python 3.12 ile, upstream CI parity olarak doğrulanır. Sonra 3.13 compatibility smoke değerlendirilir. Baseline doğrulanmadan bu dosyalar değiştirilmez.
...
## 7. NT2-02 — Reputation Domain tasarımı
...
## 11. Exact next execution

Bir sonraki oturumda yalnız `devam` denirse:

1. Bu dosyayı source-of-truth kabul et.
2. `sunsetfly/netitibar-core/main` ve foundation branch drift'ini doğrula.
3. Upstream BrightBean head değişmişse otomatik sync etme; yalnız bilgi olarak diff'i kaydet.
4. Foundation branch'in açık PR durumunu doğrula.
5. Local/native test çalıştırma imkanı varsa Python 3.12 + PostgreSQL parity baseline'ını çalıştır.
6. Baseline local yeşil olmadan reputation feature koduna geçme.
7. Foundation gate kapandıktan sonra `agent/nt2-02-reputation-domain` branch/PR'ını exact foundation head'den başlat.
8. Önce domain + isolation tests; ardından Google review provider.
